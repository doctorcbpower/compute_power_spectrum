...
#else
  hdf5_status=H5Aread(hdf5_attribute, H5T_NATIVE_ULLONG, header->nall);
#endif
...
#else
          fprintf(stdout,"Species %d : n_in_file %lld, n_in_snapshot %lld\n",i,header->npart[i],header->nall[i]);
#endif
...
#else
        hdf5_status=H5Aread(hdf5_attribute, H5T_NATIVE_ULLONG, npart);
#endif
        hdf5_status=H5Aclose(hdf5_attribute);
        hdf5_status=H5Gclose(hdf5_headergrp);
